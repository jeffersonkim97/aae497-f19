...
#ifndef GAZEBO_PLUGINS_F16PLUGIN_HH_
#define GAZEBO_PLUGINS_F16PLUGIN_HH_
...
namespace gazebo
{
  ...
  typedef const boost::shared_ptr<const msgs::F16> ConstF16Ptr;
  /// \brief Allow moving the control surfaces of a F16 plane. This
  ...
  /// "~/<model_name>/control" The expected value is a F16 message.
  ///
  /// The plugin will advertise the following topic with the current state:
  /// "~/<model_name>/state"
  class GAZEBO_VISIBLE F16Plugin : public ModelPlugin
  {
    /// \brief Constructor.
    public: F16Plugin();
    ...
    private: void OnControl(ConstF16Ptr &_msg);

    /// \brief Update PID Joint controllers.
    /// \param[in] _dt time step size since last update.
    private: void UpdatePIDs(double _dt);

    /// \brief Publish F16 state.
